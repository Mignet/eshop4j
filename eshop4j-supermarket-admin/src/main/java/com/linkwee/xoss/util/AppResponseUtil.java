 ...
 *
 */
public final class AppResponseUtil {
	protected static final Logger logger = LoggerFactory.getLogger(AppResponseUtil.class);
	/**
	 * 无数据返回
	 */
	 ...
	 * 服务器异常错误
	 */
	private static final ErrorResponse errorServ =  new ErrorResponse("100001","服务器异常");

	/**
	 * 参数错误
	 */
	private static final ErrorResponse errorToken =  new ErrorResponse("100003","Token失效");
	
	/**
	 * 参数错误
	 */
	private static final ErrorResponse errorSign =  new ErrorResponse("100004","签名错误");
	/**
	 * 无数据返回
	 * @return
	 */
	 ...
	 * @return
	 */
	public static <T>BaseResponse getSuccessResponse(PaginatorResponse<?> datas,Class<T> clazz){
		return  getSuccessResponse(new PaginatorResponse<T>(datas,clazz));
	}
	...
	public static ErrorResponse getErrorParams(BindingResult... results) {
		ErrorResponse errorParams =  new ErrorResponse("100002","参数错误");
		if(results!=null&&results.length>0){
			for(BindingResult result:results){
				if(result.hasErrors()) {
	...
	public static BaseResponse getErrorBusi(BaseResponse error) {
		ErrorResponse errorBusi =  new ErrorResponse("100005","业务逻辑错误");
		List<BaseResponse> errors = new ArrayList<BaseResponse>();
		errors.add(error);
	...
	public static BaseResponse getErrorSign(BaseResponse error) {
		ErrorResponse errorBusi =  new ErrorResponse("100004","签名错误");
		List<BaseResponse> errors = new ArrayList<BaseResponse>();
		errors.add(error);
		errorBusi.setErrors(errors);
		return errorBusi;
	...
	public static ErrorResponse getErrorToken(BaseResponse error) {
		ErrorResponse errorParams =  new ErrorResponse("100003","Token失效");
		List<BaseResponse> errors = new ArrayList<BaseResponse>();
		errors.add(error);
		errorParams.setErrors(errors);
		return errorParams;
