...
//		String bankMemo = "交易备注";
	...
	@Override
	public Map<String, String> approveOnePay(List<String> listStr) {
		Map<String,String> returnStr = new HashMap<String, String>();
		List<AcWithdrawApply> list = new ArrayList<AcWithdrawApply>();
		for(String id: listStr){
			//根据ID查询提现记录
		...
		for (AcWithdrawApply apply:list) {
			//密钥
			String key = bill99config.getKey();//"TU3CATZZ4MJUUC3U";
			AcBankCardInfo card = acBankCardInfoService.selectByBankCardId(apply.getBankCardId());
			String payAmount = replaceAmount(apply.getAmount().toString());
			//组合字符串。。必须按照此顺序组串
			String macVal= card.getBankCard() + payAmount + apply.getOrderId() + key;
			String mac = null;
			try {
				mac = MD5Util.md5Hex(macVal.getBytes("gb2312")).toUpperCase();
			} catch (UnsupportedEncodingException e) {
				LOGGER.info("approveOnePay转换mac异常!");
				e.printStackTrace();
			}

			BankRequestBean requestBean = new BankRequestBean();
			requestBean.setProvince_city(apply.getCity().replace("市", ""));//城市,中文字符 主要只需要城市名，不需要省份名。也不要带"市""自治区（县）"等
			requestBean.setBankName(apply.getBankName());//银行名称 请填写银行的标准名,详见接口文档
			requestBean.setKaihuhang(apply.getKaiHuHang());//银行卡开户行的名称
			requestBean.setCreditName(apply.getUserName());//收款人姓名,收款人的姓名，必须与银行卡账户名相同
			requestBean.setBankCardNumber(card.getBankCard());//银行卡号  
			requestBean.setAmount(Double.parseDouble(payAmount));//交易金额  整数或小数  小数为两位以内  但小数点的最后一位不能为0 如：0.10不行。单位为人民币元  
			requestBean.setDescription("领会提现");//交易备注
			requestBean.setOrderId(apply.getOrderId());//订单号
			requestBean.setMac(mac);
			BankRequestBean[] queryArray = new BankRequestBean[1];
			queryArray[0] = requestBean;

			String merchant_id = bill99config.getMerchantId();//"10012138842";
			String merchant_ip = bill99config.getMerchantIp();//"183.3.219.162";
			...
			try {
				...
				LOGGER.info("approveOnePay【RemoteException】异常!");
				returnStr.put("code", "approveOnePay【RemoteException】异常!");
			} catch (ServiceException e) {
				LOGGER.info("approveOnePay【ServiceException】异常!");
				returnStr.put("code", "approveOnePay【ServiceException】异常!");
			}
			if(responseBean!=null&&responseBean[0]!=null){
					AcWithdrawApply newApply = new AcWithdrawApply();
					newApply.setOrderId(apply.getOrderId());
					newApply.setDealId(responseBean[0].getDealId());//快钱交易号
					newApply.setDebitCharge(null);
					newApply.setResultCode(responseBean[0].getFailureCause());//原因代码
					newApply.setStatus("0000".equals(responseBean[0].getFailureCause())?"2":"6");
		    		
		    		if(!"0000".equals(responseBean[0].getFailureCause())){
			    		AcWithdrawApply acw = new AcWithdrawApply();
			    		acw.setOrderId(responseBean[0].getOrderId());
			    		//原提现申请记录
			    		AcWithdrawApply with = acWithdrawApplyMapper.selectOneByCondition(acw);
			    		AcAccountBind bind = new AcAccountBind();
			    		bind.setUserId(with.getUserId());
				    	...
				    	if("0".equals(with.getIsThaw())){
				    		...
				    		newApply.setStatus("7");
				    	}
		    		}
		    		newApply.setUpdatedDate(new Date());
		    		newApply.setUpdatedBy(currentUser != null?JSON.toJSONString(currentUser.getPrincipal()):null);
		    		newApply.setConfirmTime(new Date());//审核时间
		    		newApply.setRemark(responseBean[0].getFailureCause());
	...
	public String replaceAmount(String amount){
		if(amount.contains(".")&&"0".equals(amount.substring(amount.length()-1, amount.length()))){
			amount = amount.substring(0, amount.length()-1);
		}
		if(".".equals(amount.substring(amount.length()-1, amount.length()))){
			amount = amount.substring(0, amount.length()-1);
		}
		if(amount.contains(".")&&"0".equals(amount.substring(amount.length()-1, amount.length()))){
			amount = replaceAmount(amount);
		}
		return amount;
	...
	@Override
	@Transactional
	public void queryWithdrawforJob(){
		//1.根据条件查询需要查询的批次号 status in('2','8') and result_code='101'
		List<AcWithdrawApply> list = acWithdrawApplyMapper.queryWithdrawforJob();
		
		
		//消息推送
		final List<SysMsg> lcsSysMsg = Lists.newArrayList();
		final List<SmMessageQueue> lcsMessage = Lists.newArrayList();
		
		final List<SysMsg> invSysMsg = Lists.newArrayList();
		final List<SmMessageQueue> invMessage = Lists.newArrayList();
		final List<SysPushMessage> invPushMsg = Lists.newArrayList();
		//消息模板
		SmMessageTemplate condit = new SmMessageTemplate();
		condit.setModuleId(MsgModuleEnum.FINISHWITHDRAWAL.getValue());
			...
			if(bidqr!=null&&bidqr.getResponseBody()!=null&&bidqr.getResponseBody().getBatchList()!=null&&
		    	...
		    	for(Pay2bankResultType pay: bidqr.getResponseBody().getBatchList().getPay2bankLists()){
			    	...
			    	if("111".equals(pay.getStatus())&&"0".equals(with.getIsThaw())){
			    		...
			    		if(with != null && with.getUserType()!=null && messageTmp != null ){
			    			String content = null;
			    			if(with.getUserType().intValue() == 1){//理财师
			    				content = String.format(messageTmp.getContent(),with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM),with.getAmount() == null ? "" : with.getAmount().setScale(2,BigDecimal.ROUND_DOWN),"猎财大师");
			    				lcsSysMsg.add(sysMsgService.fillSysMsg(1,PersonalMsgTypeEnum.WITHDRAWALS_INV,with.getUserId(),content));
			    				lcsMessage.add(smMessageQueueService.fillSmMessageQueue(1,with.getMobile(),content,MsgModuleEnum.FINISHWITHDRAWAL));
			    			}else{
			    				content = String.format(messageTmp.getContent(),with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM),with.getAmount() == null ? "" : with.getAmount().setScale(2,BigDecimal.ROUND_DOWN),"T呗");
			    				invSysMsg.add(sysMsgService.fillSysMsg(2,PersonalMsgTypeEnum.WITHDRAWALS_INV,with.getUserId(),content));
			    				invMessage.add(smMessageQueueService.fillSmMessageQueue(2,with.getMobile(),content,MsgModuleEnum.FINISHWITHDRAWAL));
			    				invPushMsg.add(new SysPushMessage(with.getUserId(),"提现处理完成",content));
			    			}
			    		...
			    		wxreq.setUseId(with.getUserId());
			    		wxreq.setTemkey(SysConfigConstant.WITHDRAWALS_ACCOUNT);//提现到账通知
			    		wxreq.setWithdrawTime(DateUtils.format(with.getBisTime(),DateUtils.FORMAT_LONG));
			    		wxreq.setNoticeTime(formatNoticeTime(pay.getEndDate()));
			    		wxreq.setAccount(rebind.getBankName()+" (尾号"+rebind.getBankCard().substring(rebind.getBankCard().length()-4, rebind.getBankCard().length())+")");
			    		wxreq.setWithdrawAmount(NumberUtils.getFormat(with.getAmount(), "0.00")+"元");
			    		wxreq.setUseType(with.getUserType().toString());
...
//		    		newapply.setNoticeTime(date);
		...
		ThreadpoolService.execute(new Runnable() {			
			...
			public void run() {
				if(lcsSysMsg.size() > 0){
					smMessageQueueService.batchSendDiffContentMessageAndPmsg(lcsMessage, true,lcsSysMsg);
				}
				
				if(invSysMsg.size() > 0){
					smMessageQueueService.batchSendDiffContentMessageAndPmsg(invMessage, true,invSysMsg);
					pushMessageHelper.BatchSinglePush(AppTypeEnum.INVESTOR, SmsTypeEnum.WITHDRAWRECORD, null, invPushMsg, null);
				}			
	...
	private String formatNoticeTime(String endDate){
		if(endDate!=null&&endDate.length()==14){
			endDate = endDate.substring(0, 4)+"-"+endDate.substring(4, 6)+"-"+endDate.substring(6, 8)+" "+endDate.substring(8, 10)+":"+endDate.substring(10, 12)+":"+endDate.substring(12, 14);
		}
		return endDate;
	}
	...
	@Override
	@Transactional
	public void queryOnePayWithdrawforJob(){
		//1.根据条件查询需要查询的批次号 status in('2','8') and result_code='0000'
		List<AcWithdrawApply> list = acWithdrawApplyMapper.queryOnePayWithdrawforJob();
		...
		for(AcWithdrawApply apply:list){
			...
			String merchant_id = bill99config.getMerchantId();
			String merchant_ip = bill99config.getMerchantIp();
			QueryRequestBean query = new QueryRequestBean();
	        query.setDealId(apply.getDealId());//快钱交易号
	        query.setOrderId(apply.getOrderId());//我们自己生成的订单号
			...
			if(responseBean!=null&&responseBean[0]!=null){
	    		...
	    		AcWithdrawApply newapply = new AcWithdrawApply();
	    		AcWithdrawApply acw = new AcWithdrawApply();
	    		acw.setOrderId(apply.getOrderId());
	    		//原提现申请记录
	    		AcWithdrawApply with = acWithdrawApplyMapper.selectOneByCondition(acw);
	    		//用户绑卡账户
	    		AcAccountBind rebind = acAccountBindService.selectAccountByUserId(with.getUserId());
	    		AcAccountBind bind = new AcAccountBind();
	    		bind.setUserId(with.getUserId());
	    		String dealStatus = responseBean[0].getDealStatus();
		    	//状态成功后，添加明细记录到银行账户记录表tac_balance_record  
	    		//同时将冻结的金额、提现的金额清除
		    	if("111".equals(dealStatus)&&"0".equals(with.getIsThaw())){
		    		AcBalanceRecord bala = new AcBalanceRecord();
		    		bala.setUserType(with.getUserType());
		    		bala.setUserId(with.getUserId());
		    		bala.setBankCardId(with.getBankCardId());
		    		...
		    		if(with != null && with.getUserType()!=null && messageTmp != null ){
		    			if(with.getUserType().intValue() == 1){//理财师
		    				lcsSysMsg.add(sysMsgService.fillSysMsg(1,PersonalMsgTypeEnum.WITHDRAWALS_INV,with.getUserId(),String.format(messageTmp.getContent(),with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM))));
		    				lcsMessage.add(smMessageQueueService.fillSmMessageQueue(1,with.getMobile(),String.format(messageTmp.getContent(),with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM)),MsgModuleEnum.FINISHWITHDRAWAL));
		    			}else{
		    				invSysMsg.add(sysMsgService.fillSysMsg(2,PersonalMsgTypeEnum.WITHDRAWALS_INV,with.getUserId(),String.format(messageTmp.getContent(),with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM))));
		    				invMessage.add(smMessageQueueService.fillSmMessageQueue(2,with.getMobile(),String.format(messageTmp.getContent(), with.getBisTime() == null ? "" : DateUtils.format(with.getBisTime(), DateUtils.FORMAT_SHORT_CN_MM)),MsgModuleEnum.FINISHWITHDRAWAL));
		    			}
		    		...
		    		wxreq.setTemkey(SysConfigConstant.WITHDRAWALS_ACCOUNT);//提现到账通知
		    		wxreq.setWithdrawTime(DateUtils.format(with.getBisTime(),DateUtils.FORMAT_LONG));
		    		wxreq.setNoticeTime(responseBean[0].getDealEndDate());
		    		wxreq.setAccount(rebind.getBankName()+"尾号: "+rebind.getBankCard().substring(rebind.getBankCard().length()-4, rebind.getBankCard().length()));
		    		wxreq.setWithdrawAmount(NumberUtils.getFormat(with.getAmount(), "0.00"));
		    		wxreq.setUseType(with.getUserType().toString());
		    		weiXinMsgService.sendWeiXinMsgCommon(wxreq);
		    		
		    	}
		    	//失败需要解冻
		    	if(("112".equals(dealStatus)||"114".equals(dealStatus))&&"0".equals(with.getIsThaw())){
		    		//总金额=原金额+提现总金额  补回金额
		    		BigDecimal oldTotalAmount = new BigDecimal(rebind.getTotalAmount());
		    		BigDecimal oldFreezeAmount = new BigDecimal(rebind.getFreezeAmount());
		    		BigDecimal oldWithdrawAmount = new BigDecimal(rebind.getWithdrawAmount());
		    	...
		    	//更新提现记录信息
	    		newapply.setResultCode(responseBean[0].getFailureCause());//原因代码
	    		newapply.setStatus(dealStatus);
	    		newapply.setUpdatedDate(new Date());
	    		newapply.setOrderId(responseBean[0].getOrderId());
	    		newapply.setRemark(responseBean[0].getFailureCause());
//	    		newapply.setNoticeTime(DateUtils.parse(responseBean[0].getDealEndDate(),DateUtils.FORMAT_LONG));
	    		newapply.setNoticeTime(new Date());
	    		acWithdrawApplyMapper.updateByPrimaryKeySelective(newapply);
			}
		}
