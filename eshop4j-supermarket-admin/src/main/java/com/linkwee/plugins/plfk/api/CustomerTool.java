...
public class CustomerTool {
	...
	public Object unseal(SettlementPkiApiResponse response,DealInfoEntity dealInfo) {
		...
		if (response == null) {
			LOGGER.info("BILL99[SettlementPkiApiResponse]应答信息为空");
			return null;
		} else {
			try {
	...
	private SealedData seal(DealInfoEntity dealInfo) {
		...
		LOGGER.info("BILL99提交的原始报文为:{}",originalData);
...
//	}
	...
	public byte[] unsealData(SettlementPkiApiResponse response1,DealInfoEntity dealInfo){
		if(response1==null){
			LOGGER.info("BILL99_unsealData得到的应答报文为空");
			return null;
		}else{
			try {
				String errorCode="";
				...
				LOGGER.info("BILL99_unsealData付款商户号{}",responsebody.getMemberCode());
				LOGGER.info("BILL99_unsealData应答状态{}",responsebody.getStatus());
				if(!"0000".equals(errorCode) && !"1313".equals(errorCode)){
					LOGGER.info("BILL99_unsealData申请失败,失败代码{}",errorCode);
					return null;
				}
				if("0000".equals(errorCode)){
					LOGGER.info("BILL99_unsealData申请成功");
				}else{
					LOGGER.info("BILL99_unsealData申请失败,失败代码:{}",errorCode);
				}				
				...
				if (unsealedData.getVerifySignResult()) {
					...
					LOGGER.info("BILL99_unsealData验签失败");
					return null;
				}
			} catch (CryptoException e) {
				e.printStackTrace();
