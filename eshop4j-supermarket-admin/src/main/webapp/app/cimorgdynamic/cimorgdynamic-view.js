$(function(){
	...
	$('#releaseTime').daterangepicker({
		...
		startDate: moment().format('YYYY-MM-DD HH:mm:ss'), // 默认当前时间
		minDate: moment().format('YYYY-MM-DD')	//可选最早时间
	},
	function(start, end, label) { //回调
		$("#releaseTime").closest('.form-group').removeClass('has-error'); //删除验证错误样式
		$("#releaseTime-error").remove(); //移除jqueryValidate span错误提示标签
	...
	$("#orgDynamicForm").validate({
	    ...
	    submitHandler: function (form) {
	    	...
	    	var releaseTime = $("#releaseTime").val(); //发布时间
	    	$.ajax({
	        	data: { "id":id ,"orgNumber":orgNumber , "orgName":orgName  , "orgDynamicUrl":orgDynamicUrl , "orgTitle":orgTitle , "orgContent":orgContent , "dynamicrdo":dynamicrdo ,"releaseTime":releaseTime}, //表单对象数据转json字符串提交
