...
public class CRC16Hash {
	 ...
	 * @return
	 */
	private static int doCrc(int reg_init, byte[] message) {
	 ...
	 * @return int 返校验码
	 */
	private static int doCrc(byte[] message) {
		// 计算CRC校验时初值从0x0000开始。
		int crc_reg = 0x0000;
		return doCrc(crc_reg, message);
	}
	/**
	 * db44检验方法
	 * 
	 ...
	 * @return
	 */
	private static boolean doCrc(byte[] message,byte[] crc) {
		// 计算CRC校验时初值从0x0000开始。
		int crc_reg = 0x0000;
		int crc_value = (crc[0]&0xff)*256+(crc[1]&0xff);
		return crc_value ==doCrc(crc_reg, message);
	}
	/**
	 ...
	 * @return
	 */
	public static boolean doCrc_db44(byte[] messages) {
		// 计算CRC校验时初值从0x0000开始。
		...
		return doCrc(messageArray, crcArray);
	}

	public static void main(String[] args) {
		// 一个db44测试样本数据
		String test = "e31b126dad51b310c6e5b287c8ec14dd";
		byte p[] = StringUtils.getBytesUtf8(test);
		int crc = doCrc(p); // 计算前两位的CRC码
		// 65336
