...
import com.xiaoniu.mybatis.paginator.support.PropertiesHelper;
import com.xiaoniu.mybatis.paginator.support.SQLHelp;
...
public class PageInterceptor implements Interceptor{
    ...
    public Object intercept(final Invocation invocation) throws Throwable {
        ...
        final Object objParameter = queryArgs[PARAMETER_INDEX];
       
        if(!(objParameter instanceof PageRequest)){
        	return invocation.proceed();
        }
        PageRequest request = (PageRequest)objParameter;
        //添加跳过分页设置
        if(request.getPage()==-1){
        	return invocation.proceed();
        }
        
        final Object parameter = request.getQuery();
        final PageBounds pageBounds = new PageBounds(request);
