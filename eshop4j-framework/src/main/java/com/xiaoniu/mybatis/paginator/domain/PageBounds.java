...
public class PageBounds extends RowBounds implements Serializable {
    ...
    public PageBounds(PageRequest request) {
    	this.page = request.getPage();
        this.limit = request.getRows();
        if(null!=request.getSort()&&!"".equals(request.getSort())&&
        		null!=request.getOrder()&&!"".equals(request.getOrder())){
        	if(null!=request.getOrderExpr()&&!"".equals(request.getOrderExpr())){
        		this.orders.add(Order.create(request.getSort(), request.getOrder(),request.getOrderExpr()));
        	}else{
        		if(request.getSort().charAt(0)== '"'&&request.getSort().charAt(request.getSort().length()-1)== '"'){
        	       this.orders.add(Order.create(request.getSort().substring(1, request.getSort().length()-1), request.getOrder(),"\"?\""));
	        	}else{
	        	   this.orders.add(Order.create(request.getSort(), request.getOrder()));
	        	}
        	}
        }
        this.containsTotalCount = request.isContainsTotalCount();
        this.asyncTotalCount = request.getAsyncTotalCount();
    }
